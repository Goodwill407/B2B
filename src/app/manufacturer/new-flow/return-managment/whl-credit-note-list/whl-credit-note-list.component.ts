...
@Component({
  selector: 'app-whl-credit-note-list',
  standalone: true,
  imports: [
    CommonModule,
    TableModule,
  ...
  templateUrl: './whl-credit-note-list.component.html',
  styleUrl: './whl-credit-note-list.component.scss'
})
export class WhlCreditNoteListComponent implements OnInit {
  ...
  // Route parameters
  wholesalerEmail: string = '';  // ✅ from route params (groupKey)
  used: boolean = false;
  ...
  ngOnInit(): void {
    // Manufacturer is logged in
    this.manufacturerEmail = this.authService.currentUserValue.email;

    // Get wholesaler email from route params (groupKey)
  ...
  getCreditNotes() {
    ...
    const url = `m-w-credit-note?manufacturerEmail=${this.manufacturerEmail}&wholesalerEmail=${this.wholesalerEmail}&used=${this.used}&page=${this.currentPage}&limit=${this.rows}&sortBy=createdAt:desc`;
  ...
  getPageTitle(): string {
    return this.used ? 'Used Credit Notes for Wholesaler' : 'Credit Notes for Wholesaler';
  }
}
