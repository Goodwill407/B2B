...
@Component({
  selector: 'app-return-order-ret-list',
  standalone: true,
  imports: [
    CommonModule,
    TableModule,
  ...
  templateUrl: './return-order-ret-list.component.html',
  styleUrl: './return-order-ret-list.component.scss'
})
export class ReturnOrderRetListComponent implements OnInit {
  ...
  getReturnOrders() {
    ...
    // API endpoint with manufacturer email parameter for return orders
    const mfgEmail = this.authService.currentUserValue.email;
    this.currentPage = Math.floor(this.first / this.rows) + 1;

    const url = `return-r2m?manufacturerEmail=${mfgEmail}&page=${this.currentPage}&limit=${this.rows}&sortBy=createdAt:desc`;
