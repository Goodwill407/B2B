...
@Component({
  selector: 'app-whl-credit-note-usage-view',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './whl-credit-note-usage-view.component.html',
  styleUrl: './whl-credit-note-usage-view.component.scss'
})
export class WhlCreditNoteUsageViewComponent implements OnInit {
  ...
  getWalletDetails() {
    this.isLoading = true;
    const url = `m-to-w-wallet/${this.walletId}`;  // ✅ wholesaler endpoint
  ...
  downloadPDF() {
    ...
    try {
      ...
      // Left Column — wholesaler instead of retailer
      doc.text('Wholesaler Email:', 14, 40);
      doc.setFont('helvetica', 'bold');
      doc.text(this.walletData.wholesalerEmail, 50, 40);
      ...
      autoTable(doc, {
        ...
        didParseCell: (data) => {
          if (data.column.index === 2 && data.cell.section === 'body') {
            ...
            if (cellValue === 'CREDIT') data.cell.styles.textColor = [40, 167, 69];
            else if (cellValue === 'DEBIT') data.cell.styles.textColor = [220, 53, 69];
          }
          ...
          if (data.column.index === 3 && data.cell.section === 'body') {
            ...
            if (rowType === 'CREDIT') data.cell.styles.textColor = [40, 167, 69];
      ...
      // ✅ wholesalerEmail in filename
      const sanitizedEmail = this.walletData.wholesalerEmail.replace('@', '_at_').replace(/\./g, '_');
