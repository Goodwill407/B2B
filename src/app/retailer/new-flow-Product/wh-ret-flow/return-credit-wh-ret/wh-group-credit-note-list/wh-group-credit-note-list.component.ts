...
@Component({
  selector: 'app-wh-group-credit-note-list',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './wh-group-credit-note-list.component.html',
  styleUrl: './wh-group-credit-note-list.component.scss'
})
export class WhGroupCreditNoteListComponent implements OnInit {
  ...
  getAvailableCreditNotes() {
    ...
    const url = `w-r-credit-note/group?retailerEmail=${retailerEmail}&page=${page}&limit=${this.limit}&used=false`;
    ...
    this.authService.get(url).subscribe(
      (res: any) => {
        if (res.success && res.result) {
          ...
          console.log('Available WHL Credit Notes:', this.availableData);
        }
      },
      (error) => {
  ...
  getUsedCreditNotes() {
    ...
    const url = `w-to-r-wallet?retailerEmail=${retailerEmail}&page=${page}&limit=${this.limit}`;
    ...
    this.authService.get(url).subscribe(
      (res: any) => {
        if (res && res.results) {
          ...
          console.log('WHL Wallet Data:', this.usedData);
        }
      },
      (error) => {
