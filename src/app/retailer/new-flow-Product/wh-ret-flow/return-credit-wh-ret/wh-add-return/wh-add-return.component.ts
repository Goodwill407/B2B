...
@Component({
  selector: 'app-wh-add-return',
  standalone: true,
  imports: [CommonModule, FormsModule, TableModule, RouterModule],
  templateUrl: './wh-add-return.component.html',
  styleUrl: './wh-add-return.component.scss'
})
export class WhAddReturnComponent implements OnInit {
  ...
  get RETURN_PERIOD_DAYS(): number {
    return WhAddReturnComponent.RETURN_PERIOD_DAYS;
  }

  getOrderDetails() {
    this.loading = true;

    const url = `pi-wholesaler-to-retailer/${this.orderId}`;
  ...
  validateReturnPeriod(): void {
    ...
    this.remainingReturnDays = WhAddReturnComponent.RETURN_PERIOD_DAYS - daysDifference;
    this.isReturnPeriodValid = daysDifference < WhAddReturnComponent.RETURN_PERIOD_DAYS;

    if (!this.isReturnPeriodValid) {
      setTimeout(() => {
        Swal.fire({
          title: 'Return Period Expired',
          html: `The return period of ${WhAddReturnComponent.RETURN_PERIOD_DAYS} days has expired for this order.<br>
  ...
  async onAddToReturn(item: DeliveryItem) {
    ...
    if (!this.isReturnPeriodValid) {
      Swal.fire({
        title: 'Return Period Expired',
        html: `The return period of ${WhAddReturnComponent.RETURN_PERIOD_DAYS} days has expired.<br>
               <small>You cannot add items for return.</small>`,
        icon: 'error',
    ...
    const imageDisplay = item.colourImage && item.colourImage !== ''
      ? `<img src="${item.colourImage}" alt="${item.colourName}" 
              style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; border: 2px solid #ddd;">`
      : `<div style="width: 80px; height: 80px; background-color: ${item.colour || '#e0e0e0'}; 
              border-radius: 8px; border: 2px solid #ddd; display: inline-block;"></div>`;
      ...
      html: `
        <div class="swal-product-details">
          ...
          <div class="form-group" id="custom-reason-group" style="display: none;">
            ...
            <textarea id="swal-custom-reason" class="swal2-textarea" 
                      placeholder="Enter custom reason..." style="margin: 0;"></textarea>
          </div>
        </div>
      `,
      focusConfirm: false,
  ...
  private updateReturnQuantities(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const updateUrl = `pi-wholesaler-to-retailer/${this.orderId}`;
      const updateData = {
        ...this.responseData,
