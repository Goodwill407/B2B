...
@Component({
  selector: 'app-wh-returned-product-list',
  standalone: true,
  imports: [
    CommonModule,
    TableModule,
  ...
  templateUrl: './wh-returned-product-list.component.html',
  styleUrl: './wh-returned-product-list.component.scss'
})
export class WhReturnedProductListComponent implements OnInit {
  ...
  getReturnOrders() {
    ...
    const url = `return-r2w?retailerEmail=${retailerEmail}&page=${this.currentPage}&limit=${this.rows}&sortBy=createdAt:desc`;
  ...
  // Helper method to format status — all ReturnR2W schema statuses covered
  ...
  // Helper method to get status badge class — all ReturnR2W schema statuses covered
