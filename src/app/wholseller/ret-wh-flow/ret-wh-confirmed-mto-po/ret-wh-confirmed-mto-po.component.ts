...
@Component({
  selector: 'app-ret-wh-confirmed-mto-po',
  standalone: true,
  imports: [CommonModule, FormsModule, AccordionModule, TableModule, RouterModule, IndianCurrencyPipe, AmountInWordsPipe],
  templateUrl: './ret-wh-confirmed-mto-po.component.html',
  styleUrl: './ret-wh-confirmed-mto-po.component.scss'
})
export class RetWhConfirmedMtoPoComponent implements OnInit {
  purchaseOrder: any = {
    supplierName: '',
    supplierDetails: '',
    supplierAddress: '',
  ...
  // Wholesaler-specific properties
  ...
  getAllProducts(poId: string) {
    ...
    this.authService.get(url).subscribe(
      (res: any) => {
        ...
        // Basic PO info - Wholesaler to Retailer
        this.purchaseOrder = {
          ...
          supplierPAN: this.extractPanFromGstin(res.wholesaler.GSTIN) || res.wholesaler.PAN || '',
  ...
  async fetchWalletBalance(): Promise<any> {
    const whEmail = this.wholesalerProfile.email;
    const retEmail = this.purchaseOrder.buyerEmail;
    
    const url = `w-to-r-wallet?wholesalerEmail=${whEmail}&retailerEmail=${retEmail}`;
  ...
  async debitWalletBalance(walletId: string, amount: number, invoiceNumber: number): Promise<void> {
    ...
    try {
      const url = `w-to-r-wallet/debit/${walletId}`;
  ...
  async createInvoiceWithCredit(creditAmount: number) {
    ...
    const invoicePayload = {
      ...
      wholesalerEmail: this.wholesalerProfile.email,
      ...
      wholesaler: this.wholesalerProfile,
      retailer: {
    ...
    try {
      ...
      const invoiceResponse = await this.authService.post('pi-wholesaler-to-retailer', invoicePayload).toPromise();
