...
@Component({
  selector: 'app-ret-wh-credit-note-view',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './ret-wh-credit-note-view.component.html',
  styleUrl: './ret-wh-credit-note-view.component.scss'
})
export class RetWhCreditNoteViewComponent implements OnInit {
  ...
  getCreditNoteDetails() {
    this.loading = true;
    const url = `w-r-credit-note/${this.creditNoteId}`;

    this.authService.get(url).subscribe(
      (res: any) => {
        this.creditNoteData = res;
        this.loading = false;
        console.log('WHL Credit Note Details:', this.creditNoteData);
      },
      (error) => {
