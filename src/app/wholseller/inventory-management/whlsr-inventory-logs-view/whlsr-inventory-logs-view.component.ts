...
@Component({
  selector: 'app-whlsr-inventory-logs-view',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './whlsr-inventory-logs-view.component.html',
  styleUrl: './whlsr-inventory-logs-view.component.scss'
})
export class WhlsrInventoryLogsViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Get logData via navigation state
    ...
    this.authService.get(
      `wholesaler-inventory?userEmail=${currentUser.email}&search=${encodeURIComponent(this.logData.designNumber)}`
    ).subscribe((res: any) => {
  ...
  downloadPDFTable() {
    ...
    doc.text(`Wholesaler Inventory Log Report`, 105, 15, { align: 'center' });
    ...
    for (let key of this.groupedKeys) {
      ...
      const columns = ['Status', 'Reason', 'Qty Changed', 'Prev Qty', 'Updated Date', 'Updated By'];
    ...
    doc.save(`${this.logData?.designNumber || 'unknown'}_wholesaler_inventory-log.pdf`);
  }
}
