...
export class WhUpdateStockOfProductComponent {
  product: any = null;
  inventoryStock: any[] = [];
  ...
  getProductDetails() {
    ...
    const inventoryUrl = `wholesaler-inventory${params}`;
  ...
  proceedWithDumpStock(design: any, dumpReason: string) {
    ...
    this.authService.post('wholesaler-inventory/bulk', dumpedEntries).subscribe({
      next: () => {
        // Update local UI
        ...
        if (logPayload.length > 0) {
          this.authService.post('wholesaler-inventory-logs', logPayload).subscribe({
            next: () => {
  ...
  proceedWithStockUpdate(design: any, updatedEntries: any[], removalReasons: string[]) {
    ...
    const stockToSubmit = updatedEntries.map((entry: any) => ({
      ...entry,
      lastUpdatedBy: currentUser,
      lastUpdatedAt: now
    }));
